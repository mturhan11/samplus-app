...
export class TanimlamaMusterilerComponent {
  ...
  kayitGuncelle(payload:HttpParams){
    const result =  this.httpClient.post(environment._baseUrl+'/ws/ws-clients.php?aksiyon=guncelle',payload)
    return result;
  }

  kayitEkle(payload:HttpParams){
    const result =  this.httpClient.post(environment._baseUrl+'/ws/ws-clients.php?aksiyon=kaydet',payload)
    return result;
  }
  ...
  constructor(private httpClient : HttpClient) {
    ...
    this.dataStoreMusteriler = new CustomStore({
      ...
      insert: (values) => {
        ...
        if (values.musteri_adi!=null)
        {
          payload = payload.append("musteri_adi",values.eposta);
        } 
        if (values.aktif != null)
        {
          payload = payload.append("aktif",values.eposta);
        }
        return new Promise((resolve, reject) => {
          that.kayitEkle(payload).subscribe((results)=>{
            const _result = JSON.parse(JSON.stringify(results));
              ...
              if (!isOk) {
               ...
               {
                const message = 'Müşteri başarı ile güncellendi.';// ${that.currentEmployee.FirstName} ${that.currentEmployee.LastName}`;
                notify({
                  message,
                  position: {
                    my: 'center bottom',
      ...
      remove: (key) => {
        ...
        payload = payload.append("token",localStorage.getItem('token')!);
        return this.httpClient.delete(environment._baseUrl+'/ws/ws-clients.php?aksiyon=sil&'+payload).toPromise().then();
      },
      update: (key, values) => {
        let payload = new HttpParams();
        payload = payload.append("id",key);
        ...
        if (values.musteri_adi!=null)
        {
          payload = payload.append("musteri_adi",values.musteri_adi);
        } 
        if (values.aktif != null)
        {
  ...
  this.saveButtonOptions = {
    ...
    async onClick(e: ClickEvent){
      ...
      {
        let { aktif, musteri_adi} = that.formVar?.formData;
        let payload = new HttpParams();
        payload = payload.append("token",localStorage.getItem('token')!);
        if (musteri_adi!=null)
        {
          payload = payload.append("musteri_adi",musteri_adi);
        } 
        if (aktif != null)
        {
...
@NgModule({
  ...
  declarations: [TanimlamaMusterilerComponent],
  bootstrap: [TanimlamaMusterilerComponent],
})
export class TanimlamaMusterilerModule { }