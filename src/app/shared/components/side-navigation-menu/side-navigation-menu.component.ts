...
import { navigation_yonetici, navigation_kullanici , navigation_satinalma} from '../../../app-navigation';
...
export class SideNavigationMenuComponent implements  AfterViewInit, OnDestroy, OnInit {
  ...
  @Output()
  menuItems = new EventEmitter<string[]>();

  @Output()
  ArrayMenu = new EventEmitter<String[]>();
  ...
  menuListele(){
    ...
    return this.httpClient.post(environment._baseUrl+'/ws/ws-navbar.php?aksiyon=list',payload);
  }
  ...
  ngOnInit(): void {
    this.menuListele().subscribe(data => {
      const arrayMenu = JSON.parse(JSON.stringify(data));
      if (!this._items) {
        this._items = arrayMenu.map((item) => {
